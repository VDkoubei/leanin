...
/**
 * The template for displaying Author Archive pages.用于展示作者归档的模板
 *
 * Used to display archive-type pages for posts by an author.用来展示某一个作者的文章
 *
	...
	<section id="primary" class="site-content">
		...
		<?php if ( have_posts() ) : ?>
			...
			<?php
				 ...
				 * 当处理第一篇文章的时候，我们才知道正在应对的作者是谁
				 * 
				 * We reset this later so we can run the loop
				 * properly with a call to rewind_posts().
				 * 当合理循环输出文章时候，我们将在后面重置这个
				 */
				the_post();
			?>

			<header class="archive-header">
				<h1 class="archive-title"><?php printf( __( 'Author Archives: %s', 'freshsky' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' ); ?></h1>
			...
			<?php
				 ...
				 * 既然我们改变上述的the_post()，我们需要返回到开始loop，如此更好更充分地循环文章
				 */
				rewind_posts();
			?>

			<?php freshsky_content_nav( 'nav-above' ); ?>

			<?php
			// If a user has filled out their description, show a bio on their entries.如果一个用户已经填写了他的描述，就在词条中显示简历
			if ( get_the_author_meta( 'description' ) ) : ?>
			<div class="author-info">
				<div class="author-avatar">
					<?php echo get_avatar( get_the_author_meta( 'user_email' ), apply_filters( 'freshsky_author_bio_avatar_size', 60 ) ); ?>
				</div><!-- .author-avatar -->
				<div class="author-description">
					<h2><?php printf( __( 'About %s', 'freshsky' ), get_the_author() ); ?></h2>
			...
			<?php pagination($query_string,'nav-below');  ?>
