...
function freshsky_page_readers_cache($outer,$timer,$limit){
	global $wpdb;
	$counts = $wpdb->get_results("SELECT 
									...
									FROM (
										SELECT * 
											...
											WHERE comment_date > date_sub( now(), interval $timer month ) 
											AND user_id='0' 
											AND comment_author != '{$outer}' 
											AND post_password='' 
											AND comment_approved='1' 
											and comment_type=''
											) as tempcmt 
									GROUP BY comment_author ORDER BY cnt desc limit $limit");
	foreach ($counts as $count) {
		$avatar_url = get_bloginfo('wpurl') . '/avatar/' . md5(strtolower($count->comment_author_email)).'.jpg';;
...
function freshsky_page_readers_nocache($outer,$timer,$limit){
	global $wpdb;
	$counts = $wpdb->get_results("SELECT 
									count(comment_author) AS cnt, comment_author, comment_author_url, comment_author_email 
									FROM (
	...
	foreach ($counts as $count) {
		...
		$c_url = $count->comment_author_url;
		if ($c_url == '') $c_url = '#';
		$type .= '<li><a target="_blank" href="'. $c_url . '">'.get_avatar($comment->comment_author_email, 38).'<em>' . $count->comment_author . '</em><strong>+'. $count->cnt . '</strong><br>'.substr( $c_url , 7 ).'</a></li>';
	}
	echo $type;
}
	...
	<div id="primary" class="site-content">
		<div id="content" role="main">
			...
			<?php while ( have_posts() ) : the_post(); ?>
				...
				echo '<div class="readers">';
				global $wpdb;
          		$comment_count=$wpdb->get_var("SELECT COUNT(*) FROM $wpdb->comments");
				echo '<h2 class="page-title">'.__('Leave Footprints', 'freshsky').'<span>+'.$comment_count.'</span></h2>';
				if ($h_option['general']['AvatarCache'] == 1) { 
         			freshsky_page_readers_cache($outer='浩子',$timer='30',$limit='36');
				}else{
					freshsky_page_readers_nocache($outer='浩子',$timer='30',$limit='36');
				}
  				echo '</div>';
