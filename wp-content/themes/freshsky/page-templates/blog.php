 ...
 * 这是主体中必须中必须拥有的，另一个必须拥有的是style.css
 * 这被用来展示没有匹配特定查询的页面，比如没有home.php，就会显示这个页面
 *
...
get_header(); 
$ali_cats=get_all_category_ids();
$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
$args = array(
    // 以下代码中的title就是orderby的值，按标题排序
    //'orderby'   => title,、
    'ignore_sticky_posts' => 0,
    'category__in'=> $ali_cats,
    'paged' => $paged
);
query_posts($args);
?>
		...
		<?php else : ?>
			...
			?>
				<header class="entry-header">
					<h1 class="entry-title"><?php _e( 'No posts to display', 'freshsky' ); ?></h1>
				</header>

				<div class="entry-content">
					<p><?php printf( __( 'Ready to publish your first post? <a href="%s">Get started here</a>.', 'freshsky' ), admin_url( 'post-new.php' ) ); ?></p>
				</div><!-- .entry-content -->
			...
			?>
				<header class="entry-header">
					<h1 class="entry-title"><?php _e( 'Nothing Found', 'freshsky' ); ?></h1>
				</header>

				<div class="entry-content">
					<p><?php _e( 'Apologies, but no results were found. Perhaps searching will help find a related post.', 'freshsky' ); ?></p>
					<?php get_search_form(); ?>
				</div><!-- .entry-content -->
