...
?>
<?php if ( is_single() ) ://--------------------------------------------------- ?>
<article id="post-<?php the_ID(); ?>" class="thumb-single-article<?php foreach(get_post_class() as $a)echo " ".$a;?>">
		<header class="entry-header">
			<h1 class="entry-title"><?php the_title(); ?></h1>
			<?php if ( $h_option['page_post']['SingleAdsTop'] != '' ) {
				echo '<div class="single-ads-top" style="text-align:center;width:100%;">';
				echo $h_option['page_post']['SingleAdsTop'];
				echo '</div><!-- .single-ads-top/ -->';
			} ?>
			<figure class="thumb-show">
				<?php freshsky_get_thumb_image("show", 320); ?>
			</figure>
					
			<div id="share_toolbar" style="width:100%;">
				<div title="<?php _e('The views of This article', 'freshsky') ?>" id="stb_article_view" class="stb_group">
				...
				<?php 
					if (get_previous_post($categoryIDS)) {
						previous_post_link('<span id="stb_btn_prev">%link</span>','%title',true);
					} else { 
						echo '<span id="stb_btn_prev"><a href="javascript:alert(\'你正在看的文章已经是最新的了...\');" title="上一篇：木有了"></a></span>';
					} 
				?>
				<?php 
					if (get_next_post($categoryIDS)) {
				 		next_post_link('<span id="stb_btn_next">%link</span>','%title',true);
					} else {
						echo '<span id="stb_btn_next"><a href="javascript:alert(\'你正在看的文章已经是最老旧的了...\');" title="下一篇：木有了"></a></span>';
					 } 
				?> 
					</div>
					<div class="stb_divide"></div>
				<?php if ( $h_option['page_post']['ShareCode'] != '' ) {
					echo $h_option['page_post']['ShareCode'];
				} ?>
		...
		<div class="entry-content">
			...
			<?php wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'freshsky' ), 'after' => '</div>' ) ); ?>
		</div><!-- .entry-content/文章内容 -->

		<footer class="entry-meta">
			<ul> 
				<li><?php _e("Categories","freshsky") ?>: <?php the_category(' | ') ?></li> 
				<li><?php _e("Tags","freshsky"); ?>: <?php the_tags('' , ' , ' , ''); ?></li>								  
				<li><?php _e("Popular Heat","freshsky"); ?>: 超过 <span id="post_view_count"><?php if(function_exists('the_views')) { the_views(); } ?></span> 人围观</li>
				<li><?php _e("Writing Date","freshsky"); ?>: <?php the_time("Y年m月j日 - G时h分s秒"); ?></li>
				<li><?php _e("Loading Time","freshsky"); ?>: 加载本文共用时 <?php timer_stop(1); ?> 秒</li>
				<li><?php _e("Post Link","freshsky"); ?>: <a href="<?php the_permalink() ?>"><?php the_permalink() ?></a> [<a href="#" onclick="copy_code('<?php the_permalink() ?>'); return false;">复制</a>] (转载时请注明本文出处及文章链接)</li>
			</ul> 
		</footer><!-- .entry-meta/正文内容 -->
	</article><!-- #post -->
<?php else : ///===============================================?>
	<article id="post-<?php the_ID(); ?>" class="thumb-little-article<?php foreach(get_post_class() as $a)echo " ".$a;?>">
	<?php if ( is_sticky() && is_home() && ! is_paged() ) : ?>
		<div class="featured-post">
			<?php _e( 'Featured post', 'freshsky' ); ?>
		</div>
	<?php endif; ?>
	<figure class="thumb-little">
		<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>" target="_blank"><?php freshsky_get_thumb_image(); ?></a>
	</figure>
	<header class="entry-header">
					
			<h1 class="entry-title">
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'freshsky' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
			</h1>
		<?php if ( !comments_open() ) : ?>
			<div class="comments-link">
				<?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a reply', 'freshsky' ) . '</span>', __( '1 Reply', 'freshsky' ), __( '% Replies', 'freshsky' ) ); ?>
			</div><!-- .comments-link -->
		...
		<?php endif; ?>
		<footer class="entry-meta">
			<?php freshsky_entry_meta(); ?>
			<?php edit_post_link( __( 'Edit', 'freshsky' ), '<span class="edit-link">', '</span>' ); ?>
		</footer><!-- .entry-meta -->
	</article><!-- #post -->
<?php endif; // is_single() ?>
