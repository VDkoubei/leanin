...
/**
 * The template for displaying Search Results pages.搜索页面
 *
 * @package WordPress
	...
	<section id="primary" class="site-content">
		...
		<?php if ( have_posts() ) : ?>
			...
			<header class="page-header">
				<h1 class="page-title"><?php printf( __( 'Search Results for: %s', 'freshsky' ), '<span>' . get_search_query() . '</span>' ); ?></h1>
			</header>

			<?php freshsky_content_nav( 'nav-above' );//上面页码 ?>
			...
			<?php pagination($query_string,'nav-below'); //下面页码 ?>
		...
		<?php else : ?>
			...
			<article id="post-0" class="post no-results not-found">
				...
				<div class="entry-content">
					<p><?php _e( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'freshsky' ); ?></p>
					<?php get_search_form(); ?>
				</div><!-- .entry-content -->
			</article><!-- #post-0 -->
