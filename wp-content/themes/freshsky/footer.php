...
global $h_option;
/**
 * The template for displaying the footer.页脚模板
 *
 * Contains footer content and the closing of the
 * #main and #page div elements.在#main和#page的层元素
 *
 * @package WordPress
...
<section class="foot">
	...
	//友情链接
    $args  = array(
    	'category_before' => '<div class="footer-links">',
    	'category_after' => '</div>',
    	'title_before' => '<h3>',
    	'title_after' => '</h3>',
    	'orderby' =>id ,
    	'categorize' =>0 ,
    	'title_li' =>__("Blogroll", 'freshsky')
    	);
    if (is_home()  || is_page_template( 'page-templates/cms.php' ) ) wp_list_bookmarks($args); ?>
	<?php
	//底部菜单
	$args =array( 
	...
	<footer id="colophon" role="contentinfo">
		<div class="site-info">
			<p><?php do_action( 'freshsky_credits' ); ?>
			<?php 

				//默认显示内容
			 if ($h_option['general']['footerContent'] == "") {
    	
				$utility_text=__('The copyright is owned by <a href="%1$s" title="%2$s">%3$s</a>', 'freshsky');
				printf(
					$utility_text,
					get_bloginfo('url'),
				...
				echo " | ";
				$utility_text=__('Theme designed by <a href="%1$s" title="%2$s">%3$s</a>', 'freshsky');
				printf(
					$utility_text,
					'http://www.0day8.com',
					'大学之情，IT风云！',
				...
				echo " | ";
				$utility_text=__('Build based on <a href="%1$s" title="%2$s">%3$s</a>', 'freshsky');
				printf(
					$utility_text,
					'http://wordpress.org/',
					__( 'Semantic Personal Publishing Platform', 'freshsky' ),
					'WordPress'
				);
				//=====统计代码
				if ( $h_option['general']['analytics'] =="") {
					echo " | ";
					_e('Website Analytics',"freshsky"); //===================
				}else{
					echo $h_option['general']['analytics'];
				}//统计代码
					
			}else{
				 	echo $h_option['general']['footerContent'];
				 }
				?></p>
		</div><!-- .site-info -->
	</footer><!-- #colophon -->
