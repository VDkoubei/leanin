...
global $h_option;
/**
 * The Template for displaying all single posts.文章页面
 *
 * @package WordPress
				...
				<nav class="nav-single">
					...
					<?php
						if (get_previous_post($categoryIDS)) {
							previous_post_link('<span class="nav-previous">%link</span>','<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'freshsky' ) . '</span> %title',true);
						} else { 
							echo '<span class="nav-previous"><a href="javascript:alert(\'你正在看的文章已经是最新的了...\');" title="上一篇：木有了">上一篇：木有了</a></span>';
						} 
					?>
					<?php 
						if (get_next_post($categoryIDS)) {
							next_post_link('<span class="nav-next">%link</span>','%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'freshsky' ) . '</span>',true);
						} else {
							echo '<span class="nav-next"><a href="javascript:alert(\'你正在看的文章已经是最老旧的了...\');" title="下一篇：木有了">下一篇：木有了</a></span>';
							 } 
					?> 
				</nav><!-- .nav-single -->
				<?php include(TEMPLATEPATH . '/inc/related-posts.php'); ?>
				<?php if ( $h_option['page_post']['SingleAdsBottom'] != '' ) {
					echo '<section class="single-ads-bottom" style="text-align: center; width: 100%;margin-top: 20px;">';
					echo $h_option['page_post']['SingleAdsBottom'];
					echo '</section><!-- /底部广告 -->';
				} ?>
