...
/**
 * The template for displaying image attachments.图片模板
 *
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( 'image-attachment' ); ?>>
					<header class="entry-header">
						...
						<footer class="entry-meta">
							<?php
								$metadata = wp_get_attachment_metadata();//获取附件信息，下面是打印附件的时间等信息
								printf( __( '<span class="meta-prep meta-prep-entry-date">Published </span> <span class="entry-date"><time class="entry-date" datetime="%1$s">%2$s</time></span> at <a href="%3$s" title="Link to full-size image">%4$s &times; %5$s</a> in <a href="%6$s" title="Return to %7$s" rel="gallery">%8$s</a>.', 'freshsky' ),
						...
						<nav id="image-navigation" class="navigation" role="navigation">
							<span class="previous-image"><?php previous_image_link( false, __( '&larr; Previous', 'freshsky' ) ); ?></span>
							<span class="next-image"><?php next_image_link( false, __( 'Next &rarr;', 'freshsky' ) ); ?></span>
 ...
 * 抓取在画廊里的所有图像附件的ID，所以我们可以得到在画廊里相邻的下一个或者第一个图像的URL
 * （如果我们正在查看最后一张图像画廊），或在画廊中一个图像，只是链接到该图像文件
 */
...
$k++;
// If there is more than 1 attachment in a gallery如果画廊中有超过一张图片
if ( count( $attachments ) > 1 ) :
	if ( isset( $attachments[ $k ] ) ) :
		// get the URL of the next image attachment获取下一个图片附件的地址
		$next_attachment_url = get_attachment_link( $attachments[ $k ]->ID );
	else :
		// or get the URL of the first image attachment获取第一个图片的地址
		$next_attachment_url = get_attachment_link( $attachments[ 0 ]->ID );
	endif;
else :
	// or, if there's only 1 image, get the URL of the image如果只有一张图片，就获取这个链接地址
	$next_attachment_url = wp_get_attachment_url();
endif;
?>
								<a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php
								$attachment_size = apply_filters( 'freshsky_attachment_size', array( 960, 960 ) );
