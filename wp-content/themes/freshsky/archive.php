...
/**
 * The template for displaying Archive pages.本模板是为了展示归档文章
 *
 * Used to display archive-type pages if nothing more specific matches a query.如果没有单独提供特定查询选项(如tag,category)，用于展示归档文章
 * For example, puts together date-based pages if no date.php file exists.举例：如果date.php（按日期归档文章文件，本模板将展示基于时间归档的文章列表）
 *
 * If you'd like to further customize these archive views, you may create a
 * new template file for each specific one. For example, Fresh Sky already
 * has tag.php for Tag archives, category.php for Category archives, and
 * author.php for Author archives.
 * 如果你想自顶一个特定的文章视图，请你为特定归档形式创建一个新文件。
 * 例如2012已经为标签归档创建了tag.php,为分类归档创建了category.php，为作者创建了author.php
 *
		...
		<?php if ( have_posts() ) : ?>
			<header class="archive-header">
				<h1 class="archive-title"><?php
					if ( is_day() ) :
						printf( __( 'Daily Archives: %s', 'freshsky' ), '<span>' . get_the_date() . '</span>' );
					elseif ( is_month() ) :
						printf( __( 'Monthly Archives: %s', 'freshsky' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'freshsky' ) ) . '</span>' );
					elseif ( is_year() ) :
						printf( __( 'Yearly Archives: %s', 'freshsky' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'freshsky' ) ) . '</span>' );
					else :
						_e( 'Archives', 'freshsky' );
					endif;
				?></h1>
			...
			while ( have_posts() ) : the_post();
				 ...
				 * 为内容填充特定的文章模板，如果你想在子主题中创建，请你创建一个content-___.php
				 * (其中 ___是文章类型)，然后就可以用了。
				 */
			...
			pagination($query_string,'nav-below'); 
			?>
