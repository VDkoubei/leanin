...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<?php 
			$video_url = get_post_custom_values('_post_video');

			$video_data =get_video_data($video_url['0']);?>
		<header class="entry-header">
			<?php $video_play='<div class="video-play">'.get_the_title().'</div>' ?>
			<?php 
			if ($h_option["general"]["ColorBox"] !="0") {
				if ($video_url != "") {
					$imgsrc=$video_data['img'];
					$img = ($imgsrc) ? '<img src="'.$video_data['img'].'" alt="'.$video_data['title'].'">' : freshsky_get_thumb_image("show", 320, true)  ;;
					echo '<a href="'.$video_data['swf'].'"title="'.get_the_title().'" class="waterwall-box">'.$video_play.'
						'.$img.'
						</a>'; 
				}else{
					$video_url = get_video_data(get_the_content());
					$video_href = ($video_url['swf'] != "") ? $video_url['swf']  : get_permalink();
				 ?>
					<a href="<?php echo $video_href ?>" title="<?php the_title(); ?>" class="waterwall-box" ><?php echo $video_play; freshsky_get_thumb_image("show", 320); ?></a>
				<?php }
			}else{
				if ($video_url) {
					echo '<a href="'.get_permalink().'" class="waterwall-box">'.$video_play.'
						<img src="'.$video_data['img'].'" alt="'.$video_data['title'].'" class="waterwall-box">
						</a>'; 
				}else{ ?>
					<a href="<?php echo get_permalink() ?>" title="<?php the_title(); ?>"><?php echo $video_play; freshsky_get_thumb_image("show", 320); ?></a>
				<?php }
			}
			 ?>
		...
		<?php 
		if ($h_option['page_post']['VideoMeta_can'] == 1) {?>
