...
global $h_option;
/**
 * The template for displaying Category pages.用于展示分类归档的模板
 *
...
if (in_array($cat_ID, $video_cats))  $cats_now = "video_cat";
//视频墙分类
$pic_cats = explode(',',$h_option['page_post']['PicCats_final']);
if (in_array($cat_ID, $pic_cats))  $cats_now = "pic_cat";
$h_option["general"]['masonry'] = true;
?>
		...
		<?php
		 if ( have_posts() ) : ?>
			<header class="archive-header">
				<h1 class="archive-title"><?php printf( __( 'Category Archives: %s', 'freshsky' ), '<span>' . single_cat_title( '', false ) . '</span>' ); ?></h1>
			...
			switch ($cats_now) {
				case 'video_cat':
					echo '<div class="watarfall">';
					while ( have_posts() ) : the_post();
						get_template_part( 'content', 'mvideo' );
					endwhile;
					echo '</div><!-- .watarfall/瀑布流 -->';
					break;

				case 'pic_cat':
					echo '<div class="watarfall">';
					while ( have_posts() ) : the_post();
						get_template_part( 'content', 'mpic' );
					endwhile;
					echo '</div><!-- .watarfall/瀑布流 -->';
					break;
