...
class Kriesi_comments_new_widget extends WP_Widget {
	...
	{	
		...
		echo sprintf('<div class="comments-all-count">'.sprintf(__("Leave %s footprints",'freshsky'), $comment_count)).'</div>';
		echo '<ul>';
		//开始
		global $wpdb;
		$my_email ="'" . $email . "'";
		...
		if ($h_option['general']['AvatarCache'] == 1) { 
			...
			foreach ($comments as $comment) {
				...
				$output .= "\n<li><img width='32' height='32' src='". $imgsrc ."'  alt=\"$comment->comment_author\" class='avatar'/>$comment->comment_author:<br /><a href=\"" . get_permalink($comment->ID) ."#comment-" . $comment->comment_ID . "\" title=\"" .__("Link to ", 'freshsky').$comment->post_title . "\">" . strip_tags($comment->com_excerpt)."</a></li>";
			}
			$output .= $post_HTML;
			...
			$comments = $wpdb->get_results($sql);
			$output = $pre_HTML;
			foreach ($comments as $comment) {$output .= "\n<li>".get_avatar(get_comment_author_email(), 32).strip_tags($comment->comment_author).":<br />" . " <a href=\"" . get_permalink($comment->ID) ."#comment-" . $comment->comment_ID . "\" title=\"".__("Link to ", 'freshsky') .$comment->post_title . "\">" . strip_tags($comment->com_excerpt)."</a></li>";}
			$output .= $post_HTML;
			$output = convert_smilies($output);
			echo $output;
		}
