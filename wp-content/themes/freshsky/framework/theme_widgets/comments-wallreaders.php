	...
	{	
		...
		if ($h_option['general']['AvatarCache'] == 1) { 
			...
			foreach ($counts as $count) {
				...
				$mostactive .= '<li class="mostactive">' . '<a href="'. $c_url . ' " title="' . $count->comment_author . ' ('.   sprintf(__("Leave %s footprints",'freshsky'),$count->cnt ) .')" target="_blank" rel="nofollow"><img src="' . $imgsrc . '" alt="' . $count->comment_author . '" class="avatar" /></a></li>'."\n";
			}
			echo $mostactive;
		}else{
			$sql = "SELECT DISTINCT 
			...
			foreach ($wall as $comment){
				if( $comment->comment_author_url )
					...
					$url="#";
				$r="rel='nofollow'";
				$tmp = "<li><a href='".$url."' ".$r." title='".$comment->comment_author." (".sprintf(__("Leave %s footprints",'freshsky'),$comment->cnt)."'>".get_avatar($comment->comment_author_email, 38)."</a></li>";
				$output .= $tmp;
			}
				echo $output ;
		}
