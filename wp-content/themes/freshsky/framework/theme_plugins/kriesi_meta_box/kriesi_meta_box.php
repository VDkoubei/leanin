...
##################################################################
	var $options; 			// options passed by the option file 通过选项文件中的选项
	var $boxinfo;			// meta box info passed by the option file 通过选项文件中的元框信息
	...
	# 如果这些文件被调用，就增加javascript和css到head区域
	######################################################################
	...
	# 设置新的目标为了输入内容 这个meta标签更容易地被jQuery读取
	######################################################################	
	...
	######################################################################
	# Add the meta boxes to the page/post or link 增加meta box到page/post或者链接页面
	# pass id, name, callback, show at page/post/link, in which area, priority
	# 返回 id, name, callback, show 在页面/文章页/链接页 在每一个区域，
	######################################################################
	...
	{	
		...
		echo '<p style="font-weight: bold;">'.$values['name'].'</p>';
